...
api.interceptors.request.use(
  function (response) {
    ...
    if (accessToken) {
      response.headers.Authorization = `${config.tokenType} ${accessToken}`;
    }
    ...
    if (response && response.status === 401) {
      if (!isAlreadyFetchingAccessToken) {
        ...
        api
          .post(config.refreshEndpoint, {
            ...
            REFRESH_TOKEN: util.getRefreshToken(),
          })
          .then((response) => {
      ...
      const retryOriginalRequest = new Promise((resolve) => {
        addSubscriber((accessToken) => {
          ...
          originalRequest.headers.Authorization = `${config.tokenType} ${accessToken}`;
