...
async function fetchBasketList() {
  ...
  const res = await api.get("/freOrd/basketList", {
...
async function onClickDelete(row) {
  ...
  await api.delete("/freOrd/deleteFreOrdBasket", {
...
function updateCount(props, evt) {
  ...
  if (!isNaN(evt.target.value)) {
    props.row.ITEM_QTY = evt.target.value;
  } else {
...
async function onClickOrderReg() {
  ...
  if (errorCnt === 0) {
    ...
    if (confirm(orderMsg)) {
      ...
      await api.post("/freOrd/orderReg", selectedItems.value);
...
bus.on("refreshFreOrdBasket", async () => {
  await fetchBasketList();
});
...
const columns = [
  ...
  { name: "수량", label: "수량", field: "ITEM_QTY", align: "right" },
