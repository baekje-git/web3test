...
/**
 * 오늘달에서 gap만큼 이전/이후 날짜를 format에 따라서 구한다.
 * gap이 음수면 과거, 양수면 미래날짜를 구한다.
 * @param {number} gap - 오늘과 구하고자 월의 차이
 * @param {string} format - 날짜포멧
 * @returns {string}
 */
export function getFromMonth(gap, format = "YYYY-MM") {
  const d = new Date();
  return moment(d.setMonth(d.getMonth() + gap)).format(format);
}
...
export function errorNotifyTop(message) {
  Notify.create({
    color: "red-5",
    textColor: "white",
    icon: "warning",
...
export function showNotifyTop(message) {
  Notify.create({
    color: "green-8",
    textColor: "white",
    icon: "cloud_done",
...
export function getToken() {
  return SessionStorage.getItem(config.storageTokenKeyName);
}

export function getRefreshToken() {
  return SessionStorage.getItem(config.storageRefreshTokenKeyName);
}

export function setToken(value) {
  SessionStorage.set(config.storageTokenKeyName, value);
}

export function setRefreshToken(value) {
  SessionStorage.set(config.storageRefreshTokenKeyName, value);
}
...
export function isUserLoggedIn() {
  return (
    !isEmpty(SessionStorage.getItem(config.userData)) &&
    !isEmpty(SessionStorage.getItem(config.storageTokenKeyName))
  );
}

export function getUserData() {
  const data = SessionStorage.getItem(config.userData);
  if (isEmpty(data)) {
    return {};
  }
...
export function setUserData(data) {
  SessionStorage.set(config.userData, getEncode(JSON.stringify(data)));
}

export function logout() {
  SessionStorage.remove(config.userData);
  SessionStorage.remove(config.storageTokenKeyName);
  SessionStorage.remove(config.storageRefreshTokenKeyName);
}
...
export function noticeFileDownload(payload) {
  console.log(payload);
  return api
    .post("/cmmnFile/boardFileDownload", payload, {
...
export function getFileSize(fileSz) {
  var n = 1024;
  var s = "";
  var kb = fileSz / n;
  var mb = kb / n;
  ...
  if (fileSz < n) {
    s = fileSz + " Bytes";
  } else if (fileSz >= n && fileSz < n * n) {
    s = kb.toFixed(2) + " KB";
  } else if (fileSz >= n * n && fileSz < n * n * n) {
    s = mb.toFixed(2) + " MB";
  } else if (fileSz >= n * n * n && fileSz < n * n * n * n) {
    s = gb.toFixed(2) + " GB";
  } else if (fileSz >= n * n * n * n) {
    s = tb.toFixed(2) + " TB";
  }
  return s;
}
...
export function getNumberFormat2(val) {
