            ...
            <div class="first_bottom">
              <div class="first_left">
                <div class="first_total" v-if="useTotal">
                  <div class="">
                    <span v-if="flag == 'returnReg'">총 반품 품목</span>
                    <span v-else>총 주문 품목</span>
                    <span>{{ totalcnt }}</span>
                    <span>건</span>
                  </div>
                  <div class="">
...
<script setup>
import { ref, defineEmits, computed, inject, onBeforeMount, watch } from "vue";
...
async function onClickBakRcptReg() {
  ...
  try {
    ...
    util.showNotifyTop("반품접수 처리되었습니다.");
    await bus.emit("mobileBtnChk2", "전체");
    await bus.emit("mobileOrder2", "전체");
    ordMemo.value = "";
    emit("refreshList");
    closeDialog();
  } catch (error) {
