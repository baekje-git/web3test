        ...
        <q-tab-panels v-model="dialog_bottom_tab" animated>
          <q-tab-panel name="first_tab" class="first_tab">
            <div class="first_grid">
              ...
              <div class="first_item">
                <template
                  v-if="first_bottom_반품수량 > 0 && first_bottom_SUBDIV != 'Y'"
                >
                  <span>반품가능수량</span>
                  <span>{{ util.getNumberFormat(first_bottom_반품수량) }}</span>
                  <span>개</span>
                </template>
                <template v-else>
                  <span></span>
...
<script setup>
import { ref, defineEmits, computed, inject, onUnmounted, watch } from "vue";
...
async function change_tab(tabName) {
  if (props.flag === "addOrdBasket") {
    ...
    // 반품 바구니 추가
    if (tabName === "second_tab") {
      await onClickAddBakBasket(tabName);
    } else {
      dialog_bottom_tab.value = tabName;
    }
  } else if (props.flag === "returnReg") {
...
// 반품 담기
async function onClickAddBakBasket(tabName) {
  const ITEM_CD = props.first_bottom_약코드;
  const LIMIT_QTY = props.first_bottom_반품수량;
  const SUBDIV = props.first_bottom_SUBDIV;
  const BASKET_QTY = props.first_bottom_BASKET;
  const RETURN_CAT = props.first_bottom_RETURN;
  ...
  if (BASKET_QTY !== "") {
    const qtySum = BASKET_QTY + EAcount.value;
  ...
  if (RETURN_CAT === "2" || RETURN_CAT === "6") {
    returnBasketData.value.basketGbCd = "03";
  } else {
    returnBasketData.value.basketGbCd = "02";
  }

  returnBasketData.value.saveItemCd = ITEM_CD;
  returnBasketData.value.subdivYn = SUBDIV;
...
const props = defineProps({
  ...
  second_bottom_SUBDIV: {
    type: String,
    default: "",
  },
  second_bottom_BASKET: {
    type: String,
    default: "",
  },
  second_bottom_RETURN: {
    type: String,
    default: "",
  },
  useAmount: {
...
watch(
  () => props.dialog_bottom_model,
  (dialog_bottom_model) => {
    if (!dialog_bottom_model) {
      emit("update:dialog_bottom_model", false);
      dialog_bottom_tab.value = "first_tab";
    }
  }
