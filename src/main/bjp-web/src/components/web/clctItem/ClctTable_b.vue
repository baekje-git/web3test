    ...
    >
      ...
      <template v-slot:top>
        <div class="q-table_wrap">
          ...
          <div class="q-table_header_sub_wrap" style="padding: 8px 0 16px">
            <div class="flex_between">
              <div class="flex_center">
      ...
      <template v-slot:body-cell-ITEM_NM_UNIT="props">
        ...
        >
          <div class="q-table_item">
            ...
            <div class="q-table_itemTitle ellipsis prd_name">
        ...
        <q-td :props="props" style="padding: 0">
          <div class="q-table_EA_wrap">
            ...
            <!-- <input
                  ...
                  class="EA_input"
                  @input="(evt) => updateCount(props, evt)"
                /> -->
...
async function onClickAddBasket(row) {
  ...
  if (row.REQ_QTY > row.LIMIT_QTY) {
    alert(
      `반품 가능 수량은 ${row.LIMIT_QTY}개 입니다. 다시 확인하시기 바랍니다.`
    );
    return;
  }

  if (row.EXP_DT.length != 8) {
    row.EXP_DT = "";
  }

  //반품 라디오버튼 분기처리
  if (row.SUBDIV_YN == "Y" && row.TAX_YN == "Y" && !isRet(row.ITEM_NM_UNIT)) {
    returnCat.value = 6;
    basketData.value.basketGbCd = "03";
  } else if (row.SUBDIV_YN == "N" && row.TAX_YN == "Y") {
    returnCat.value = 1;
    basketData.value.basketGbCd = "02";
  } else if (row.SUBDIV_YN == "Y" && row.TAX_YN == "Y") {
    returnCat.value = 2;
    basketData.value.basketGbCd = "03";
  } else if (row.SUBDIV_YN == "N" && row.TAX_YN == "N") {
    returnCat.value = 3;
    basketData.value.basketGbCd = "02";
  } else if (
    row.SUBDIV_YN == "Y" &&
    row.TAX_YN == "Y" &&
    isRet(row.ITEM_NM_UNIT)
  ) {
    returnCat.value = 4;
    basketData.value.basketGbCd = "02";
  }
  ...
  basketData.value.saveItemQty = row.REQ_QTY;

  await api.post("/bakRcpt/addBasket", basketData.value);
  await bus.emit("refreshHeaderBasket");

  util.showNotify(
    `${row.ITEM_NM_UNIT} ${basketData.value.saveItemQty}개를 반품 장바구니에 담았습니다.`
  );
  await bus.emit("refreshRetrunBasket");

  const confirmMsg =
    "선택한 품목이 반품 장바구니에 담겼습니다.\n반품 장바구니로 이동 하시겠습니까?";
  if (!confirm(confirmMsg)) return;

  await router.push({ path: "/return" });

  // const res = await api.get("/clctItm/bakRcptBasketCnt", {
  //   params: searchParam.value,
  // });

  // if (res > 0) {
  //   alert("반품 장바구니에 등록된 품목입니다.");
  //   return;
  // }

  //await bus.emit("addShpmntItem", row);
}
...
async function fetchItemList() {
  ...
  loading.value = true;
  const res = await api.get("/clctItm/shpmntItem", {
    params: searchParam.value,
  });
  searched.value = true;
  loading.value = false;
