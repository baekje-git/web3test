    ...
    <div class="table_b_header">
      <div class="header_left">
        ...
        <div class="table_btn">
          ...
          >
            <q-badge rounded color="#384dd6" floating>
              ?
              <q-tooltip
                class="tooltip"
                anchor="center right"
...
const totalCost = computed(() => {
  ...
  if (selectedItems.value.length > 0) {
    ...
    // selectedItems.value.reduce((acc, selectedItem) => {
    ...
    //     Number(acc) +
    //     (selectedRow
    //       ? Number(selectedRow.ORD_WP2_AMT) *
    //         Number(selectedRow.ITEM_QTY ?? selectedRow.ORGNL_ITEM_QTY)
    //       : 0)
    //   );
    // }, 0);
  }
...
async function onClickDelete(row) {
  ...
  deleteBasketParam.value.saveItemGbCd = row.ITEM_GB_CD;
  deleteBasketParam.value.saveItemCd = row.ITEM_CD;
  deleteBasketParam.value.dlvBrchCd = row.DLV_BRCH_CD;

  await api.delete("/ord/deleteComOrdBasket", {
    params: deleteBasketParam.value,
  });
...
const columns = [
  ...
  { name: "제약사", label: "제약사", field: "MAKER_NM", align: "center" },
  {
    name: "단가",
    label: "단가",
    field: "ORD_WP2_AMT",
