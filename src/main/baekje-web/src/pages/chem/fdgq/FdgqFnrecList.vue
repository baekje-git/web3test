    ...
    <q-page-container class="first-container">
      <q-banner dense :class="$q.dark.isActive ? '' : ''" class="search-wrap">
        <div class="q-gutter-md row items-start">
          ...
          <dev>
            <span>시험일자</span>
            <q-input filled type="date" v-model="startWorkDate" />~
            <q-input filled type="date" v-model="endWorkDate" />
          </dev>
          <dev>
            <span>제조번호</span>
...
export default defineComponent({
  ...
  setup() {
    ...
    const startWorkDate = ref("");
    const endWorkDate = ref("");
    const prodNo = ref("");
    const qnaYn = ref("");
    const chargeNm = ref("");
    const mandateNm = ref("");
    ...
    const searchList = (page = 1) => {
      loading.value = true;
      return api
        .post("/fdgq/fnrec/fdgqFnrecList", {
          START_PROD_DT: startProdDate.value,
          END_PROD_DT: endProdDate.value,
          START_WORK_DT: startWorkDate.value,
          END_WORK_DT: endWorkDate.value,
