...
public class SuggestionsServiceImpl implements SuggestionsService {
	...
	public void saveVoteResult(List<HashMap<String, Object>> hList) throws Exception {
		...
		for(HashMap<String, Object> row : hList) {
		        if(check==0) {
			    //이전투표결과삭제
			    cmmnDao.delete("k01.knotice.deleteK01_NOTICE_VOTE_RESULT", row);
		        }
		        if(!"".equals(row.get("REG_USER"))){
		        //새로운 투표결과 입력
				cmmnDao.insert("k01.knotice.insertK01_NOTICE_VOTE_RESULT", row);
		        }
		        check++;
	    }
	}
	...
	public void saveNoticeReg( HashMap<String, Object> hList,String ipaddr,String userAgent) throws Exception {
		...
		//공지사항채번 selectK01_NOTICESequence
		int maxNoticeSeq=0;
    	
		maxNoticeSeq=(int)cmmnDao.select("k01.knotice.selectK01_NOTICESequence", noticeMap);
		noticeMap.put("BOARD_SEQ", maxNoticeSeq);
		noticeMap.put("USER_AGENT",userAgent);
		noticeMap.put("IP_ADDR", ipaddr);
		//공지사항 입력 insertK01_NOTICE
		cmmnDao.insert("k01.knotice.insertK01_NOTICE", noticeMap);
		...
		for(HashMap<String, Object> row : voteitemlist) {
			...
			if(check==0) {
		    //이전투표item삭제	        	
		    cmmnDao.delete("k01.knotice.deleteK01_NOTICE_VOTE_ITEM", row);
	        }
	        
	        if(!"".equals(row.get("REG_USER"))){
	        //새로운 투표item 입력
			cmmnDao.insert("k01.knotice.insertK01_NOTICE_VOTE_ITEM", row);
	        }
	        check++;
    }
	}
	...
	public void saveNoticeEdit( HashMap<String, Object> hList,String ipaddr,String userAgent) throws Exception {
		...
		//공지사항 입력 insertK01_NOTICE
		cmmnDao.insert("k01.knotice.updateK01_NOTICE", row);
		...
		for(HashMap<String, Object> row : voteitemlist) {
			...
			if(check==0) {
		    //이전투표item삭제	        	
		    cmmnDao.delete("k01.suggestions.deleteK01_NOTICE_VOTE_ITEM", row);
	        }
	        
	        //if(!"".equals(row.get("REG_USER"))){
	        //새로운 투표item 입력
			cmmnDao.insert("k01.suggestions.insertK01_NOTICE_VOTE_ITEM", row);
	        //}
	        check++;
    }
	}
