...
public class MenuServiceImpl implements MenuService {
	...
	@Override
	public void saveMenuLower(List<HashMap<String, Object>> hList) throws Exception {
		for(HashMap<String, Object> row : hList) {
			...
			if(ConstantProp.GV_ROW_STATUS_ADD.equals(status)) {
				cmmnDao.insert("k99.menu.insertLowerMenu", row); 
			}else if(ConstantProp.GV_ROW_STATUS_UPDATE.equals(status)) {
				cmmnDao.update("k99.menu.updateLowerMenu", row);
			}else if(ConstantProp.GV_ROW_STATUS_DELETE.equals(status)) {
				cmmnDao.delete("k99.menu.deleteLowerMenu", row);
			}
		}
	}
