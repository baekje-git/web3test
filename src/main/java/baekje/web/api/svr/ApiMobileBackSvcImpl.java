 ...
 */
@Service
public class ApiMobileBackSvcImpl implements ApiMobileBackSvc {
    ...
    public int insertApiHistory(HashMap<String, Object> paramMap) throws Exception {
		paramMap.put(Gv.QUERY_ID  , "apiMobileBack.insertApiHistory");
    ...
    public HashMap<String, Object> putBack(HashMap<String, Object> paramMap) throws Exception {
		...
		if (listBack != null && listBack.size() > 0) {
			...
			for (HashMap<String, Object> backData : listBack) {
				backData.put("API_BRCH_CD", paramMap.get("API_BRCH_CD"));
				backData.put("API_INS", paramMap.get("API_INS"));
				backData.put("ORDER_NO", paramMap.get("ORDER_NO"));
				backData.put("CUST_CODE", paramMap.get("CUST_CODE"));
				log.debug("apiMobileBack backData:"+backData);

				backData.put(Gv.DATASOURCE, Gv.SAL1);
				backData.put(Gv.NAMESPACE , "apiMobileBack");
				backData.put(Gv.QUERY_ID  , "checkAvailStock");
				checkMessage = (String)commonDao.select(backData);
				if (!"".equals(checkMessage)) {
					hMapResult.put("RESULT", "ERROR");
					hMapResult.put("MESSAGE", backData.get("ITEM_CODE") + ":" + checkMessage);
					log.error("checkAvailStock ERROR:"+backData.get("ITEM_CODE") + ":" + checkMessage);
					return hMapResult;
				}
			}
			*/
			...
			for (HashMap<String, Object> backData : listBack) {
				...
				backData.put(Gv.QUERY_ID  , "apiMobileBack.insertOrder");
			...
			cmmnDao.callProcedure("apiMobileBack.callPBackI",pMap);
			...
			if(!pMap.get("O_VAL").equals("SUCCESS")) {
				...
				log.error("callProc callPBackI ERROR:"+pMap.get("O_MSG").toString().substring(0,len));
	        }
