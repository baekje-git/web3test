 ...
 */
@Service
public class ApiOptimaSvcImpl implements ApiOptimaSvc {
    ...
    public int insertApiHistory(HashMap<String, Object> paramMap) throws Exception {
		paramMap.put(Gv.QUERY_ID  , "apiOptima.insertApiHistory");
        return cmmnDao.insert(paramMap);
    }

    @Override
    public long selectLoggerSeq(HashMap<String, Object> paramMap) throws Exception {
		paramMap.put(Gv.QUERY_ID  , "apiOptima.selectLoggerSeq");
		return (long)cmmnDao.select(paramMap);
    }

    @Override
    public String checkAuth(HashMap<String, Object> paramMap) throws Exception {
		paramMap.put(Gv.QUERY_ID  , "apiOptima.checkAuth");
		return (String)cmmnDao.select(paramMap);
    }

    @Override
    public String checkCust(HashMap<String, Object> paramMap) throws Exception {
		paramMap.put(Gv.QUERY_ID  , "apiOptima.checkCust");
		return (String)cmmnDao.select(paramMap);
    }
    ...
    public HashMap<String, Object> putCust(HashMap<String, Object> paramMap) throws Exception {
		...
		if (paramMap.get("CUST_CODE") == "") {
			putGubun = "I";
			//신규거래처코드
			paramMap.put(Gv.QUERY_ID  , "apiOptima.selectNewCustCd");
			paramMap.put("CUST_CODE",(String)cmmnDao.select(paramMap));
			log.debug("apiOptima newCustCode:"+paramMap.get("CUST_CODE"));

			//거래처 등록
			paramMap.put(Gv.QUERY_ID  , "apiOptima.insertCust");
			putCount = cmmnDao.insert(paramMap);

			//거래처 저장 후처리 프로시저 호출			
			paramMap.put("I_TYPE"    , "I");
			cmmnDao.callProcedure("apiOptima.callPCustAft",paramMap); 
			...
			if(!paramMap.get("O_VAL").equals("SUCCESS")) {
				...
				log.error("callProc callPCustAft ERROR:"+paramMap.get("O_MSG").toString().substring(0,len));
	        }
		...
		//기존거래처
			...
			putGubun = "U";
			//거래처 등록
			paramMap.put(Gv.QUERY_ID  , "apiOptima.updateCust");
    ...
    public HashMap<String, Object> putOrder(HashMap<String, Object> paramMap) throws Exception {
		...
		//주문번호중복체크
		paramMap.put(Gv.QUERY_ID  , "apiOptima.checkOrderNo");
		...
		if (listOrder != null && listOrder.size() > 0) {
			...
			for (HashMap<String, Object> orderData : listOrder) {
				...
				log.debug("apiOptima orderData:"+orderData);

				orderData.put(Gv.QUERY_ID  , "apiOptima.checkAvailStock");
			...
			for (HashMap<String, Object> orderData : listOrder) {
				...
				orderData.put(Gv.QUERY_ID  , "apiOptima.insertOrder");
			...
			cmmnDao.callProcedure("apiOptima.callPSaleMallOrd", pMap);
